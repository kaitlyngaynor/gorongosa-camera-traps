...
For some reason, the termite mound layers are in the correct resolution, but the tree and pan focals are not stacking properly—the resolution is slightly off. So, we resample them here and re-export at correct resolution.
```{r resample focal rasters}
# if needed
termites.crop <- raster("gis/Rasters for stacking/termites.crop.tif")

# trees
tree_100m <- raster("gis/Rasters for stacking/Focal rasters/tree_100m.tif")
tree_250m <- raster("gis/Rasters for stacking/Focal rasters/tree_250m.tif")
tree_500m <- raster("gis/Rasters for stacking/Focal rasters/tree_500m.tif")
tree_1000m <- raster("gis/Rasters for stacking/Focal rasters/tree_1000m.tif")

tree_100m_res <- raster::resample(tree_100m, termites.crop, method = 'bilinear')
tree_250m_res <- raster::resample(tree_250m, termites.crop, method = 'bilinear')
tree_500m_res <- raster::resample(tree_500m, termites.crop, method = 'bilinear')
tree_1km_res <- raster::resample(tree_1000m, termites.crop, method = 'bilinear')

writeRaster(tree_100m_res, 'gis/Rasters for stacking/Focal rasters/tree_100m_res.tif', format="GTiff", overwrite=TRUE, options=c("INTERLEAVE=BAND","COMPRESS=LZW"))
writeRaster(tree_250m_res, 'gis/Rasters for stacking/Focal rasters/tree_250m_res.tif', format="GTiff", overwrite=TRUE, options=c("INTERLEAVE=BAND","COMPRESS=LZW"))
writeRaster(tree_500m_res, 'gis/Rasters for stacking/Focal rasters/tree_500m_res.tif', format="GTiff", overwrite=TRUE, options=c("INTERLEAVE=BAND","COMPRESS=LZW"))
writeRaster(tree_1km_res, 'gis/Rasters for stacking/Focal rasters/tree_1km_res.tif', format="GTiff", overwrite=TRUE, options=c("INTERLEAVE=BAND","COMPRESS=LZW"))

# pans
pans_100m <- raster("gis/Rasters for stacking/Focal rasters/pans_100m.tif")
pans_250m <- raster("gis/Rasters for stacking/Focal rasters/pans_250m.tif")
pans_500m <- raster("gis/Rasters for stacking/Focal rasters/pans_500m.tif")
pans_1000m <- raster("gis/Rasters for stacking/Focal rasters/pans_1000m.tif")

pans_100m_res <- raster::resample(pans_100m, termites.crop, method = 'bilinear')
pans_250m_res <- raster::resample(pans_250m, termites.crop, method = 'bilinear')
pans_500m_res <- raster::resample(pans_500m, termites.crop, method = 'bilinear')
pans_1km_res <- raster::resample(pans_1000m, termites.crop, method = 'bilinear')

writeRaster(pans_100m_res, 'gis/Rasters for stacking/Focal rasters/pans_100m_res.tif', format="GTiff", overwrite=TRUE, options=c("INTERLEAVE=BAND","COMPRESS=LZW"))
writeRaster(pans_250m_res, 'gis/Rasters for stacking/Focal rasters/pans_250m_res.tif', format="GTiff", overwrite=TRUE, options=c("INTERLEAVE=BAND","COMPRESS=LZW"))
writeRaster(pans_500m_res, 'gis/Rasters for stacking/Focal rasters/pans_500m_res.tif', format="GTiff", overwrite=TRUE, options=c("INTERLEAVE=BAND","COMPRESS=LZW"))
writeRaster(pans_1km_res, 'gis/Rasters for stacking/Focal rasters/pans_1km_res.tif', format="GTiff", overwrite=TRUE, options=c("INTERLEAVE=BAND","COMPRESS=LZW"))

### other pan cons layers here once they are done
panslarge_250m <- raster("gis/Rasters for stacking/Focal rasters/panslarge_250m.tif")
panslarge_250m_res <- raster::resample(panslarge_250m, termites.crop, method = 'bilinear')
writeRaster(panslarge_250m_res, 'gis/Rasters for stacking/Focal rasters/panslarge_250m_res.tif', format="GTiff", overwrite=TRUE, options=c("INTERLEAVE=BAND","COMPRESS=LZW"))

panscon_250m <- raster("gis/Rasters for stacking/Focal rasters/panscon_250m.tif")
panscon_250m_res <- raster::resample(panscon_250m, termites.crop, method = 'bilinear')
writeRaster(panscon_250m_res, 'gis/Rasters for stacking/Focal rasters/panscon_250m_ress.tif', format="GTiff", overwrite=TRUE, options=c("INTERLEAVE=BAND","COMPRESS=LZW"))

panslarge.offflood_250m <- raster("gis/Rasters for stacking/Focal rasters/panslarge.offflood_250m.tif")
panslarge.offflood_250m_res <- raster::resample(panslarge.offflood_250m, termites.crop, method = 'bilinear')
writeRaster(panslarge.offflood_250m_res, 'gis/Rasters for stacking/Focal rasters/panslarge.offflood_250m_ress.tif', format="GTiff", overwrite=TRUE, options=c("INTERLEAVE=BAND","COMPRESS=LZW"))

pans.offflood_250m <- raster("gis/Rasters for stacking/Focal rasters/pans.offflood.250m.tif")
pans.offflood_250m_res <- raster::resample(pans.offflood_250m, termites.crop, method = 'bilinear')
writeRaster(pans.offflood_250m_res, 'gis/Rasters for stacking/Focal rasters/pans.offflood_250m_ress.tif', format="GTiff", overwrite=TRUE, options=c("INTERLEAVE=BAND","COMPRESS=LZW"))
...
## pan layers
pans.crop.100m.res <- raster("GIS/Rasters for stacking/Focal rasters/pans_100m_res.tif")
pans.crop.250m.res <- raster("GIS/Rasters for stacking/Focal rasters/pans_250m_res.tif")
pans.crop.500m.res <- raster("GIS/Rasters for stacking/Focal rasters/pans_500m_res.tif")
pans.crop.1km.res <-  raster("GIS/Rasters for stacking/Focal rasters/pans_1000m_res.tif")
```
...
```{r}
# bring in focal layers again to redo them
tree_100m_res <- raster("gis/Rasters for stacking/Focal rasters/tree_100m_res.tif")
tree_250m_res <- raster("gis/Rasters for stacking/Focal rasters/tree_250m_res.tif")
tree_500m_res <- raster("gis/Rasters for stacking/Focal rasters/tree_500m_res.tif")
tree_1km_res <- raster("gis/Rasters for stacking/Focal rasters/tree_1km_res.tif")

termites_100m <- raster("gis/Rasters for stacking/Focal rasters/termites_100m.tif")
termites_250m <- raster("gis/Rasters for stacking/Focal rasters/termites_250m.tif")
termites_500m <- raster("gis/Rasters for stacking/Focal rasters/termites_500m.tif")
termites_1km <- raster("gis/Rasters for stacking/Focal rasters/termites_1000m.tif")

pans_100m_res <- raster("gis/Rasters for stacking/Focal rasters/pans_100m_res.tif")
pans_250m_res <- raster("gis/Rasters for stacking/Focal rasters/pans_250m_res.tif")
pans_500m_res <- raster("gis/Rasters for stacking/Focal rasters/pans_500m_res.tif")
pans_1km_res <- raster("gis/Rasters for stacking/Focal rasters/pans_1km_res.tif")

panslarge_250m_res <- raster("gis/Rasters for stacking/Focal rasters/panslarge_250m_res.tif")
panslarge.offflood_250m_res <- raster("gis/Rasters for stacking/Focal rasters/panslarge.offflood_250m_ress.tif")
panscon_250m_res <- raster("gis/Rasters for stacking/Focal rasters/panscon_250m_ress.tif")
pans.offflood_250m_res <- raster("gis/Rasters for stacking/Focal rasters/pans.offflood_250m_ress.tif")
```
...
tree_100m_res_mask <- mask(tree_100m_res, hexes)
writeRaster(tree_100m_res_mask, 'gis/Rasters for stacking/Masked rasters/tree_100m_res_mask.tif', format="GTiff", overwrite=TRUE, options=c("INTERLEAVE=BAND","COMPRESS=LZW"))

tree_250m_res_mask <- mask(tree_250m_res, hexes)
writeRaster(tree_250m_res_mask, 'gis/Rasters for stacking/Masked rasters/tree_250m_res_mask.tif', format="GTiff", overwrite=TRUE, options=c("INTERLEAVE=BAND","COMPRESS=LZW"))

tree_500m_res_mask <- mask(tree_500m_res, hexes)
writeRaster(tree_500m_res_mask, 'gis/Rasters for stacking/Masked rasters/tree_500m_res_mask.tif', format="GTiff", overwrite=TRUE, options=c("INTERLEAVE=BAND","COMPRESS=LZW"))

tree_1km_res_mask <- mask(tree_1km_res, hexes)
writeRaster(tree_1km_res_mask, 'gis/Rasters for stacking/Masked rasters/tree_1km_res_mask.tif', format="GTiff", overwrite=TRUE, options=c("INTERLEAVE=BAND","COMPRESS=LZW"))

termites_100m_mask <- mask(termites_100m, hexes)
writeRaster(termites_100m_mask, 'gis/Rasters for stacking/Masked rasters/termites_100m_mask.tif', format="GTiff", overwrite=TRUE, options=c("INTERLEAVE=BAND","COMPRESS=LZW"))

termites_250m_mask <- mask(termites_250m, hexes)
writeRaster(termites_250m_mask, 'gis/Rasters for stacking/Masked rasters/termites_250m_mask.tif', format="GTiff", overwrite=TRUE, options=c("INTERLEAVE=BAND","COMPRESS=LZW"))

termites_500m_mask <- mask(termites_500m, hexes)
writeRaster(termites_500m_mask, 'gis/Rasters for stacking/Masked rasters/termites_500m_mask.tif', format="GTiff", overwrite=TRUE, options=c("INTERLEAVE=BAND","COMPRESS=LZW"))

termites_1km_mask <- mask(termites_1km, hexes)
writeRaster(termites_1km_mask, 'gis/Rasters for stacking/Masked rasters/termites_1km_mask.tif', format="GTiff", overwrite=TRUE, options=c("INTERLEAVE=BAND","COMPRESS=LZW"))

pans_100m_res_mask <- mask(pans_100m_res, hexes)
writeRaster(pans_100m_res_mask, 'gis/Rasters for stacking/Masked rasters/pans_100m_res_mask.tif', format="GTiff", overwrite=TRUE, options=c("INTERLEAVE=BAND","COMPRESS=LZW"))

pans_250m_res_mask <- mask(pans_250m_res, hexes)
writeRaster(pans_250m_res_mask, 'gis/Rasters for stacking/Masked rasters/pans_250m_res_mask.tif', format="GTiff", overwrite=TRUE, options=c("INTERLEAVE=BAND","COMPRESS=LZW"))

pans_500m_res_mask <- mask(pans_500m_res, hexes)
writeRaster(pans_500m_res_mask, 'gis/Rasters for stacking/Masked rasters/pans_500m_res_mask.tif', format="GTiff", overwrite=TRUE, options=c("INTERLEAVE=BAND","COMPRESS=LZW"))

pans_1km_res_mask <- mask(pans_1km_res, hexes)
writeRaster(pans_1km_res_mask, 'gis/Rasters for stacking/Masked rasters/pans_1km_res_mask.tif', format="GTiff", overwrite=TRUE, options=c("INTERLEAVE=BAND","COMPRESS=LZW"))
...
panscon_250m_res_mask <- mask(panscon_250m_res, hexes)
writeRaster(panscon_250m_res_mask, 'gis/Rasters for stacking/Masked rasters/panscon_250m_res_mask.tif', format="GTiff", overwrite=TRUE, options=c("INTERLEAVE=BAND","COMPRESS=LZW"))

panslarge_250m_res_mask <- mask(panslarge_250m_res, hexes)
writeRaster(panslarge_250m_res_mask, 'gis/Rasters for stacking/Masked rasters/panslarge_250m_res_mask.tif', format="GTiff", overwrite=TRUE, options=c("INTERLEAVE=BAND","COMPRESS=LZW"))

pans.offflood_250m_mask <- mask(pans.offflood_250m_res, hexes)
writeRaster(pans.offflood_250m_mask, 'gis/Rasters for stacking/Masked rasters/pans.offflood_250m_mask.tif', format="GTiff", overwrite=TRUE, options=c("INTERLEAVE=BAND","COMPRESS=LZW"))

panslarge.offflood_250m_res_mask <- mask(panslarge.offflood_250m_res, hexes)
writeRaster(panslarge.offflood_250m_res_mask, 'gis/Rasters for stacking/Masked rasters/panslarge.offflood_250m_res_mask.tif', format="GTiff", overwrite=TRUE, options=c("INTERLEAVE=BAND","COMPRESS=LZW"))
...
test_stack <- raster::stack(pans.offflood_250m_mask, 
                            panscon_250m_res_mask, 
                            panslarge.offflood_250m_res_mask,
                            termites_100m_mask, termites_250m_mask, 
                            termites_500m_mask, termites_1km_mask,
                            tree_100m_res_mask, tree_250m_res_mask,
                            tree_500m_res_mask, tree_1km_res_mask,
                            pans_100m_res_mask, pans_250m_res_mask,
                            pans_500m_res_mask, pans_1km_res_mask)
```
