...
```{r import GIS}
## Park boundary
boundary.lyr <- readOGR("~/Dropbox/projects/GORONGOSA2/Camera Trap Grid/R/GIS/Spatial data/GIS layers", "gnp_boundary_west_straight_latlong") # use "." if in working directory, otherwise enter file path
boundary.lyr.UTM <- spTransform(boundary.lyr, CRS("+proj=utm +south +zone=36 +ellps=WGS84")) # project in UTM

## Roads
# roads.lyr <- readOGR("~/Dropbox/projects/GORONGOSA2/Camera Trap Grid/R/GIS/roads_curr_aug2015.kml", "roads_curr_aug2015") # this is a KML, so slightly different format - but I've edited roads since, so ignore this one
roads.lyr <- readOGR("~/Dropbox/projects/GORONGOSA2/Camera Trap Grid/R/GIS/Spatial data/GIS layers", "Roads_all")
roads.lyr.UTM <- spTransform(roads.lyr, CRS("+proj=utm +south +zone=36 +ellps=WGS84")) # project in UTM
plot(roads.lyr.UTM)

roadsmajor.lyr <- readOGR("~/Dropbox/projects/GORONGOSA2/Camera Trap Grid/R/GIS/Spatial data/GIS layers", "Roads_major")
...
## Lake Urema
urema.lyr <- readOGR("~/Dropbox/projects/GORONGOSA2/Camera Trap Grid/R/GIS/Spatial data/GIS layers", "lake_urema_latlong") # use "." if in working directory, otherwise enter file path
...
## Pans
pans.lyr <- readOGR("~/Dropbox/projects/GORONGOSA2/Camera Trap Grid/R/GIS/Spatial data/Pan", "PanOutline_130429_2")
pans.lyr.UTM <- spTransform(pans.lyr, CRS("+proj=utm +south +zone=36 +ellps=WGS84"))

## Conservative Pans (no channels and floodplain)
panscon.lyr <- readOGR("~/Dropbox/projects/GORONGOSA2/Camera Trap Grid/R/GIS/Spatial data/Pan_conservative", "PanOutline_130429_2")
...
panslarge.lyr <- readOGR("~/Dropbox/projects/GORONGOSA2/Camera Trap Grid/R/GIS/Spatial data/Pan_conservative", "Pan_largerthan1km2")
...
panslarge.offflood <- gDifference(panslarge.lyr.UTM, floodplain)
# all
pans.offflood <- gDifference(pans.lyr.UTM, floodplain)

## Settlements
settlements.lyr.UTM <- readOGR("~/Dropbox/projects/GORONGOSA2/Camera Trap Grid/R/GIS/Spatial data/Settlements", "settlements_all_proj") %>% spTransform(CRS("+proj=utm +south +zone=36 +ellps=WGS84"))
# and get chitengo in there
chitengo.lyr.UTM <- readOGR("~/Dropbox/projects/GORONGOSA2/Camera Trap Grid/R/GIS/Spatial data/Settlements", "chitengo_latlong") %>% spTransform(CRS("+proj=utm +south +zone=36 +ellps=WGS84"))
settlements.lyr.UTM <- bind(settlements.lyr.UTM, chitengo.lyr.UTM)

## Poaching
poaching.all.lyr <- readOGR("~/Dropbox/projects/GORONGOSA2/Camera Trap Grid/R/GIS/Spatial data/Poaching", "All_Poaching")
poaching.all.lyr.UTM <- spTransform(poaching.all.lyr, CRS("+proj=utm +south +zone=36 +ellps=WGS84"))

poaching.high.lyr <- readOGR("~/Dropbox/projects/GORONGOSA2/Camera Trap Grid/R/GIS/Spatial data/Poaching", "HIGHPoaching")
poaching.high.lyr.UTM <- spTransform(poaching.high.lyr, CRS("+proj=utm +south +zone=36 +ellps=WGS84"))

poaching.low.lyr <- readOGR("~/Dropbox/projects/GORONGOSA2/Camera Trap Grid/R/GIS/Spatial data/Poaching", "Low_Poaching")
...
fire2016.lyr <- readOGR("~/Dropbox/projects/GORONGOSA2/Camera Trap Grid/R/GIS/Spatial data/Fire2", "gnp_fire_2016") # use "." if in working directory, otherwise enter file path
...
tree.crop.100m <- focal(tree.crop, w = matrix(1/1201, nc = 101, nr = 101))
writeRaster(tree.crop.100m, 'tree_100m.tif', format = "GTiff", overwrite = TRUE, options = c("INTERLEAVE=BAND", "COMPRESS=LZW"))

tree.crop.250m <- focal(tree.crop, w = matrix(1/251001, nc = 251, nr = 251))
writeRaster(tree.crop.250m, 'tree_250m.tif', format = "GTiff", overwrite = TRUE, options = c("INTERLEAVE=BAND", "COMPRESS=LZW"))

tree.crop.500m <- focal(tree.crop, w = matrix(1/63001, nc = 501, nr = 501))
writeRaster(tree.crop.500m, 'tree_500m.tif', format = "GTiff", overwrite = TRUE, options = c("INTERLEAVE=BAND", "COMPRESS=LZW"))

tree.crop.1km <- focal(tree.crop, w = matrix(1/1002001, nc = 1001, nr = 1001))
writeRaster(tree.crop.1km, 'tree_1000m.tif', format = "GTiff", overwrite = TRUE, options = c("INTERLEAVE=BAND", "COMPRESS=LZW"))
...
termites.crop.100m <- focal(termites.crop, w = matrix(1/1201, nc = 101, nr = 101))
writeRaster(termites.crop.100m, 'termites_100m.tif', format = "GTiff", overwrite = TRUE, options = c("INTERLEAVE=BAND", "COMPRESS=LZW"))

termites.crop.250m <- focal(termites.crop, w = matrix(1/251001, nc = 251, nr = 251))
writeRaster(termites.crop.250m, 'termites_250m.tif', format = "GTiff", overwrite = TRUE, options = c("INTERLEAVE=BAND", "COMPRESS=LZW"))

termites.crop.500m <- focal(termites.crop, w = matrix(1/63001, nc = 501, nr = 501))
writeRaster(termites.crop.500m, 'termites_500m.tif', format = "GTiff", overwrite = TRUE, options = c("INTERLEAVE=BAND", "COMPRESS=LZW"))

termites.crop.1km <- focal(termites.crop, w = matrix(1/1002001, nc = 1001, nr = 1001))
writeRaster(termites.crop.1km, 'termites_1000m.tif', format = "GTiff", overwrite = TRUE, options = c("INTERLEAVE=BAND", "COMPRESS=LZW"))

## pans
pans.100m <- focal(pans.raster, w = matrix(1/1201, nc = 101, nr = 101))
writeRaster(pans.100m, 'pans_100m.tif', format = "GTiff", overwrite = TRUE, options = c("INTERLEAVE=BAND", "COMPRESS=LZW"))

pans.250m <- focal(pans.raster, w = matrix(1/251001, nc = 251, nr = 251))
writeRaster(pans.250m, 'pans_250m.tif', format = "GTiff", overwrite = TRUE, options = c("INTERLEAVE=BAND", "COMPRESS=LZW"))

pans.500m <- focal(pans.raster, w = matrix(1/63001, nc = 501, nr = 501))
writeRaster(pans.500m, 'pans_500m.tif', format = "GTiff", overwrite = TRUE, options = c("INTERLEAVE=BAND", "COMPRESS=LZW"))

pans.1km <- focal(pans.raster, w = matrix(1/1002001, nc = 1001, nr = 1001))
writeRaster(pans.1km, 'pans_1000m.tif', format = "GTiff", overwrite = TRUE, options = c("INTERLEAVE=BAND", "COMPRESS=LZW"))
...
hexes <- readOGR("~/Dropbox/projects/GORONGOSA2/Camera Trap Grid/R/GIS/Camera layers", "CameraGridHexes")
